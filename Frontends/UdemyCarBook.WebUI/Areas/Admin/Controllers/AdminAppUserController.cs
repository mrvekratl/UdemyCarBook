    ...
    public class AdminAppUserController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (token != null)
            {
                ...
                var responseMessage = await client.GetAsync("https://localhost:7080/api/AppUsers");
                if (responseMessage.IsSuccessStatusCode)
                {
                    var jsonData = await responseMessage.Content.ReadAsStringAsync();
                    var values = JsonConvert.DeserializeObject<List<GetAppUserList>>(jsonData);
                    return View(values);
                }
            }
            return View();
        ...
        public async Task<IActionResult> CreateAppUser(CreateAppUserDto createAppUserDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7080/api/AppUsers", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7080/api/AppUsers?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminAppUser", new { area = "Admin" });
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7080/api/AppUsers/{id}");
        ...
        public async Task<IActionResult> UpdateAppUser(UpdateAppUserDto updateAppUserDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7080/api/AppUsers/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminAppUser", new { area = "Admin" });
            }
